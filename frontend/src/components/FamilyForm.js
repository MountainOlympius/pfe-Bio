...
const FamilyForm = ({data = {}}) => {
    ...
    const addCriteriaCallback = useCallback(() => {        
        if (criteria.length <= 0 || (criteria[criteria.length - 1].content && criteria[criteria.length - 1].content !== '')) {
            setCriteria([...criteria, {}])
        }
    }, [criteria])

    const onChangeCriteriaCallback = useCallback((i, e) => {
    ...
    const saveFamily = (e) => {
        ...
        const data = {
            ...
            criteria: criteria.filter(cr => Boolean(cr.content)).map((cr) => {
		...
		<form className="FamilyForm" onSubmit={saveFamily}>
            ...
            <div className='form-div'>
                ...
                <select defaultValue={data.phylum_id} name='phylum_id'>
                    {phylums.map((phylum, i) => <option key={i} value={phylum.id}>{phylum.name}</option>)}
                </select>
            </div>
            ...
            <div className='form-div'>
                ...
                {criteria.map((cr, i) => (<div key={i} className='criteria-div'>
                    <textarea onChange={(e) => onChangeCriteriaCallback(i, e)} className='criteria' defaultValue={cr.content} />
                    <button onClick={deleteCriteriaCallback} type='button' className='delete-criteria'>supprimer critère</button>
                </div>))}

                <button type='button' onClick={addCriteriaCallback} className='add-criteria-btn'>Ajouter critère</button>
            </div>
